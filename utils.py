...
async def main_convertor_handler(
    ...
    """
    Handles message conversion with additional processing like link replacement.
    """
    try:
        if user:
            header_text = (
        ...
        # Handle reply_markup URLs
        reply_markup = await create_inline_keyboard_markup(message, method_func, user=user)

        # Add header and footer
        shortenedText = f"{header_text}\n{shortenedText}\n{footer_text}"

        # Handle media and banners
        fileid = None
        if message.media:
            medias = getattr(message, message.media.value, None)
            if hasattr(medias, "file_id"):
        ...
        # Edit or reply to the message with processed data
        if message.text:
            if edit_caption:
                try:
        ...
        logger.error(f"Error in main_convertor_handler: {e}")
        raise
...
async def set_commands(app):
    COMMANDS = [
        ...
        BotCommand("stats", "Displays statistics of the server and bot(ony for admin)."),
        BotCommand("batch", "Converts link for multiple posts (ask to admin for permit).")
